  ...
  return {
    ...
    defaults: function (config) {
      ...
      return {
        ...
        paths: {
          ...
          //For sequelize hook
          sequelize : path.resolve(config.appPath, 'api/sequelize/model'),
          // Server-Side HTML
          //
          // For `views` hook
    ...
    configure: function() {
      ...
      _.extend(sails.config.paths, {
        ...
        // For sequelize hook
        sequelize: path.resolve(sails.config.appPath, sails.config.paths.sequelize),
        // Server-Side HTML
        //
        // For `views` hook
     ...
     * @param {Function} cb
     */
    loadSequelize: function (cb) {
      console.log('##############5');
      console.log(JSON.stringify(sails.config.paths.sequelize));
      console.log('##############5');
      buildDictionary.optional({
        // dirname: sails.config.paths.controllers,
        dirname: sails.config.paths.sequelize,
        filter: /(.+)\.js$/,
        //filter: /(.+)Controller\.(js|coffee|litcoffee)$/,
        flattenDirectories: true,
        keepDirectoryPath: true,
